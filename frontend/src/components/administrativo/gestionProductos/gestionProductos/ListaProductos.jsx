...
const ListaProductos = ({ productos, categorias, descuentos, onEditar, onEliminar }) => {
  ...
  // Modificada para devolver el objeto completo del descuento
  const obtenerDescuento = (descuentoId) => {
    const descuento = (descuentos || []).find((desc) => desc.id === descuentoId);
    return descuento || null; // Devuelve null si no se encuentra el descuento
  };
          ...
          <Table variant="simple" tableLayout="fixed" width="100%">
            ...
            <Tbody>
              {productos.map((producto) => {
                const descuento = obtenerDescuento(producto.descuento_id); // Obtener el objeto de descuento
                return (
